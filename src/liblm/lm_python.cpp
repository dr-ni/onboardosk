...
{
    public:
        NGramIter(DynamicModelBase* lm_)
        {
            this->lm = lm_;
            it = lm->ngrams_begin();
            first_time = true;
    ...
    public:
        ...
        DynamicModelBase* lm;
        std::unique_ptr<DynamicModelBase::ngrams_iter> it;
        bool first_time;
};
...
NGramIter_iternext(PyObject *self)
{
    ...
    else
    {
        ...
        {
            PyObject* oword = NULL;
            const wchar_t* word = iter->lm->m_dictionary.id_to_word_w(ngram[i]);
            if (word)
            {
