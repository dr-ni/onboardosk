 ...
 */
#ifndef LM_DYNAMIC_IMPL_H
#define LM_DYNAMIC_IMPL_H
    ...
    for (int i=0; i<n; i++)
    {
        ...
        if (!node)
        {
            if (i == m_order-1)
            {
                ...
                int size = p->m_children.size();
                int old_capacity = p->m_children.capacity();
                if (size >= old_capacity)  // no space for another TLASTNODE?
                {
                    // grow the memory block of the parent node
                    int new_capacity = p->m_children.capacity(size + 1);
                    ...
                    ASSERT(p == grand_parent->m_children[grand_parent_index]);
                    grand_parent->m_children[grand_parent_index] = pnew;
                    MemFree(p);
                    p = pnew;
                }
...
void _DynamicModel<TNGRAMS>::set_order(int n)
{
    ...
    m_n1s = std::vector<int>(n, 0);
    m_n2s = std::vector<int>(n, 0);
    m_Ds  = std::vector<double>(n, 0);    // discounting parameters
...
template <class TNGRAMS>
BaseNode* _DynamicModel<TNGRAMS>::count_ngram(const std::vector<std::string>& ngram,
...
template <class TNGRAMS>
BaseNode* _DynamicModel<TNGRAMS>::count_ngram(const std::vector<const char*>& ngram,
                                              int increment, bool allow_new_words)
{
    size_t n = ngram.size();
    std::vector<WordId> wids(n);
...
template <class TNGRAMS>
BaseNode* _DynamicModel<TNGRAMS>::count_ngram(const char* const* ngram, size_t n,
                                              int increment, bool allow_new_words)
{
    std::vector<WordId> wids(n);
...
template <class TNGRAMS>
BaseNode* _DynamicModel<TNGRAMS>::count_ngram(const std::vector<const wchar_t*> ngram,
                                      int increment, bool allow_new_words)
{
    size_t n = ngram.size();
    std::vector<WordId> wids(n);
...
template <class TNGRAMS>
int _DynamicModel<TNGRAMS>::get_ngram_count(const char* const* ngram, int n)
{
                                       ...
                                       std::vector<double>& probabilities)
{
    ...
    int n = std::min((int)history.size(), m_order-1);
    std::vector<WordId> h(m_order-1, UNKNOWN_WORD_ID);
    std::copy_backward(history.end()-n, history.end(), h.end());

    #ifdef LMDEBUG
    static_assert(false);
    for (int i=0; i<m_order; i++)
        printf("%d: n1=%8d n2=%8d D=%f\n", i, m_n1s[i], m_n2s[i], m_Ds[i]);
    #endif
