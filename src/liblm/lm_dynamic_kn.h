...
class BeforeLastNodeKNBase : public TBASE
{
    public:
        ...
        int get_N1pxr() {return m_N1pxr;}

    public:
        uint32_t m_N1pxr;    // number of word types wid-n+1 that precede wid-n+2..wid in the training data
};
...
class TrieNodeKNBase : public TBASE
{
    ...
    public:
        ...
        uint32_t m_N1pxr;    // number of word types wi-n+1 that precede
                           // wi-n+2..wi in the training data
        uint32_t m_N1pxrx;   // number of permutations around center part
};
...

{
    ...
    public:
        ...
        int get_N1pxr(const BaseNode* node, int level) const;
        int get_N1pxrx(const BaseNode* node, int level) const;
                         ...
                         int increment)
{
    ...
    if (node->m_count == 0 && increment > 0)
    {
        ...
        if (!nd)
            return -1;
        ((TBEFORELASTNODE*)nd)->m_N1pxr++; // count number of word types wid-n+1
        ...
        if (n >= 2)
        {
            ...
            BaseNode* nd_ = this->add_node(wxrx);
            if (!nd_)
                return -1;
            ((TNODE*)nd_)->m_N1pxrx++;  // count number of word types wid-n+1 that precede wid-n+2..wid in the training data
        }
    }
    ...
    if (node->m_count == 0 && increment < 0)
    {
        ...
        if (!nd)
            return -1;
        ((TBEFORELASTNODE*)nd)->m_N1pxr--;

        if (n >= 2)
        {
            std::vector<WordId> wxrx(wids+1, wids+n-1);
            BaseNode* _nd = this->add_node(wxrx);
            if (!_nd)
                return -1;
            ((TNODE*)_nd)->m_N1pxrx--;
        }
...
template <class TNODE, class TBEFORELASTNODE, class TLASTNODE>
int NGramTrieKN<TNODE, TBEFORELASTNODE, TLASTNODE>::
    get_N1pxr(const BaseNode* node, int level) const
{
    if (level == this->m_order)
        return 0;
    if (level == this->m_order - 1)
        return static_cast<const TBEFORELASTNODE*>(node)->m_N1pxr;
    return static_cast<const TNODE*>(node)->m_N1pxr;
}

template <class TNODE, class TBEFORELASTNODE, class TLASTNODE>
int NGramTrieKN<TNODE, TBEFORELASTNODE, TLASTNODE>::
    get_N1pxrx(const BaseNode* node, int level) const
{
    if (level == this->m_order)
        return 0;
    if (level == this->m_order - 1)
        return 0;
    return static_cast<const TNODE*>(node)->get_N1pxrx();
}
                            ...
                            const std::vector<double>& Ds)
{
    ...
    ASSERT(static_cast<int>(history.size()) == this->m_order-1);
                ...
                if (N1pxrx)
                {
                    ...
                    if (h.size() == 0) // empty history?
                    {
                        ...
                        for(i=0; i<size; i++)
                        {
                            ...
                            TNODE* node = static_cast<TNODE*>(this->m_children[words[i]]);
                            vc[i] = node->m_N1pxr;
                        }
                    }
                    else
                    {
                        ...
                        int num_children_ = this->get_num_children(hnode, j);
                        for(i=0; i<num_children_; i++)
                        {
                            ...
                            int index = binsearch(words, child->m_word_id);
                            if (index != -1)
                                vc[index] = child->m_N1pxr;
                        }
                    }

                    double D = Ds[j];
                    double l1 = D / static_cast<double>(N1pxrx) * N1prx; // normalization factor
                                                           // 1 - lambda
                    for(i=0; i<size; i++)
                    {
                        double a = vc[i] - D;
...
class _DynamicModelKN : public _DynamicModel<TNGRAMS>
{
    ...
    public:
        _DynamicModelKN(OrderType order=3) :
            _DynamicModel<TNGRAMS>(order)
        {
        ...
        {
            std::unique_ptr<_DynamicModelKN> model = std::make_unique<_DynamicModelKN>();
                                         ...
                                         std::vector<double>& probabilities)
{
    ...
    switch(this->m_smoothing)
    {
        case KNESER_NEY_I:
            this->ngrams.get_probs_kneser_ney_i(h, words, probabilities,
                                          this->get_num_word_types(), this->m_Ds);
            break;
